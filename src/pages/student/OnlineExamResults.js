...
const OnlineExamResults = () => {
    ...
    if (loading) return (
        <div className="d-flex flex-column align-items-center justify-content-center mt-5">
            ...
            <div className="fw-bold text-primary mt-3">Loading online results...</div>
        </div>
    );
    const allClasses = [...new Set(examMetadata.flatMap(meta =>
        Array.isArray(meta.AllowedClass) ? meta.AllowedClass : [meta.AllowedClass]
    ))].filter(Boolean);

    const allExamTitles = [...new Set(availableExams)]
    if (!title) {
        return (
            <div className="container py-4 mt-5">
                <h4 className="text-primary fw-bold mb-4">
                    <i className="fas fa-list-alt me-2"></i>Select an Exam to View Result
                </h4>
                ...
                <div className="card shadow-sm border-0 mb-4">
                    <div className="card-body">
                        <h5 className="fw-bold mb-3 text-info">
                            <i className="fas fa-filter me-2"></i>Filter & Search Exams
                        </h5>
                        <div className="row">
                            <div className="col-md-4 mb-3">
                                <label className="form-label fw-semibold">
                                    <i className="fas fa-user-graduate me-1 text-secondary"></i>Filter by Class
                                </label>
                                <select
                            ...
                            <div className="col-md-4 mb-3">
                                <label className="form-label fw-semibold">
                                    <i className="fas fa-book me-1 text-success"></i>Filter by Exam Title
                                </label>
                                <select
                                    className="form-select"
                            ...
                            <div className="col-md-4 mb-3">
                                <label className="form-label fw-semibold">
                                    <i className="fas fa-search me-1 text-info"></i>Search Exam Title
                                </label>
                                <input
                                    type="text"
                        ...
                        .filter(examTitle => {
                            ...
                            const classMatch = selectedClass
                                ? (Array.isArray(meta?.AllowedClass)
                                    ? meta.AllowedClass.includes(selectedClass)
                                    : meta?.AllowedClass === selectedClass)
                                : true;
                            const examMatch = selectedExamTitle
                                ? examTitle === selectedExamTitle
                                : true;
                            const searchMatch = examTitle.toLowerCase().includes(searchTerm.toLowerCase());
                            return classMatch && examMatch && searchMatch;
                        })
                        .map((examTitle, idx) => {
                            const result = allResults.find(r => r.examTitle === examTitle && r.studentId === rollno);
                            ...
                            return (
                                <div className="col-md-4 mb-4" key={idx}>
                                    <div className="card shadow-lg border-0 h-100">
                                        ...
                                        {/* Font Icon as Image Cap */}
                                        <div className="icon-snip-wrapper bg-light text-center border-bottom">
                                            <i className="fas fa-file-alt fa-6x text-primary icon-snip"></i>
                                        </div>

                                        <div className="card-body d-flex flex-column justify-content-between">
                                            <h5 className="card-title fw-bold text-info text-center">
                                                {examTitle}
                                            </h5>
                                            ...
                                            {meta.Date && (
                                                <p className="mb-1 text-center">
                                                    ...
                                                    <strong>Exam Date:</strong> {meta.Date}
                                                </p>
                                            )}
                                            ...
                                            {isValidAttempted(result.attemptedOn) ? (
                                                <p className="mb-1 text-success text-center">
                                                    <i className="fas fa-check-circle me-2"></i>
                                                    <strong>Attempted On:</strong> {result.attemptedOn}
                                                </p>
                                            ) : (
                                                <p className="mb-1 text-danger text-center">
                                            ...
                                            <button
                                                className={`btn mt-3 w-100 ${isValidAttempted(result.attemptedOn)
                                                    ? 'btn-primary'
                                            ...
                                            >
                                                {isValidAttempted(result.attemptedOn)
                                                    ? (<><i className="fas fa-poll me-2"></i>View Result</>)
                                                    : (<><i className="fas fa-ban me-2"></i>Not Attempted</>)
                                                }
                                            </button>
                                        </div>
                                    </div>
            ...
            {currentAns && (
                <div className="card p-3 mb-4 border-start border-4 border-secondary rounded-3 shadow-sm">
                    ...
                    <div className="d-flex justify-content-between align-items-center mt-4 px-2">
                        <button
                            className={`btn btn-lg rounded-pill px-4 fw-semibold shadow-sm ${currentAnswerIndex === 0 ? 'btn-outline-secondary disabled' : 'btn-outline-secondary'
                                }`}
                        ...
                        <span className="text-muted fw-bold">
                            {currentAnswerIndex + 1} / {filteredAnswers.length}
                        </span>

                        <button
                            className={`btn btn-lg rounded-pill px-4 fw-semibold shadow-sm ${currentAnswerIndex === filteredAnswers.length - 1
